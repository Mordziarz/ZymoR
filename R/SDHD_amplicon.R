#' Analyze SDHD gene mutations from BAM files
#'
#' This function processes BAM files to identify specific mutations in the SDHD gene.
#' It maps genomic coordinates to coding sequences (CDS) by accounting for the 
#' exon structure of the SDHD gene, performs codon translation using the 
...
#'   \item{R47W, I50L}{Columns representing the detected amino acid 
#'   (or "wt", "Del", "OUT") at each target position.}
#'
#' @details
#' The function maps SDHD gene positions (located on the minus strand) and performs translation 
#' of codons using the mitochondrial genetic code (code 4).
#' - "wt" is assigned if the amino acid matches the reference.
#' - The specific mutation is labeled as defined in the target profile.
...
get_SDHD_amplicon <- function(input_bam) {
  
  .datatable.aware <- TRUE
  
  SDHD_CDS <- IRanges::IRanges(start = c(443, 264, 1), end = c(693, 383, 211))
  full_map <- unlist(lapply(seq_along(SDHD_CDS), function(i) seq(end(SDHD_CDS[i]), start(SDHD_CDS[i]))))
  
  gen_code <- Biostrings::getGeneticCode("4")
  SDHD_target_positions <- c(R47W=47, I50L=50)
  target_names <- names(SDHD_target_positions)
  ...
  for(bam_file in bam_files) {
    ...
    for (m_name in target_names) {
      pos <- SDHD_target_positions[[m_name]]
