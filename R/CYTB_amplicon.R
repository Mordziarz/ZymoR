  ...
  for(bam_file in bam_files) {
    ...
    for (m_name in target_names) {
      ...
      if (length(valid_idx) > 0) {
        c1 <- subseq(seqs_aligned[valid_idx], start = l1[valid_idx], end = l1[valid_idx])
        c2 <- subseq(seqs_aligned[valid_idx], start = l2[valid_idx], end = l2[valid_idx])
        c3 <- subseq(seqs_aligned[valid_idx], start = l3[valid_idx], end = l3[valid_idx])
